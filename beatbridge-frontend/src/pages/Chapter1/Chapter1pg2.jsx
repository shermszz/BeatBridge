...
const exercises = [
  ...
  { name: 'Exercise 3', pattern: ['R', 'L', 'L', 'R'] },
  { name: 'Exercise 4', pattern: ['R', 'R', 'L', 'R'] },
  { name: 'Exercise 5', pattern: ['L', 'L', 'R', 'L'] },
  { name: 'Exercise 6', pattern: ['R', 'R', 'L', 'R', 'L', 'L', 'R', 'L'] },
  { name: 'Exercise 7', pattern: ['R', 'R', 'L', 'L', 'R', 'R', 'L', 'L'] },
  { name: 'Exercise 8', pattern: ['R', 'R', 'R', 'L', 'R', 'R', 'R', 'L'] },
  { name: 'Exercise 9', pattern: ['L', 'L', 'L', 'R', 'L', 'L', 'L', 'R'] }
];
...
export default function Chapter1pg2() {
  ...
  useEffect(() => {
    ...
    // Play metronome click immediately
    playMetronome();
  ...
  const patternBoxes = (
    <div className={`stick-pattern-bar${canHit && isPlaying ? ' metronome-flash' : ''}`}>
      {pattern.map((s, i) => {
        let state = '';
        if (i < stepIdx) state = userHits[i] === s ? 'correct' : 'incorrect';
        else if (i === stepIdx) state = 'active';
        return (
          <div key={i} className={`stick-pattern-step ${state}`}>{s}</div>
        );
      })}
    </div>
  );
  ...
  return (
    <div className="chapter1-container">
      <h1 className="chapter1-title">Stick Control 1: Quarter Notes</h1>
      <div className="chapter1-exercise-card-nav-wrapper">
        <div className="chapter1-exercise-card">
          <div className="chapter1-description">
            Practise your stick control! For each exercise, press <b style={{color: '#ffb3b3'}}>J</b> for right hand (R) and <b style={{color: '#ffb3b3'}}>D</b> for left hand (L) <b>in time with the metronome</b>. Only hit the snare when you hear the beat. Once you are ready, press 'Play' to begin!
          </div>
          {metronomeControls}
